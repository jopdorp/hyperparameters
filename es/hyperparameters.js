import { subtract, matrix, sum, square, add, multiply, identity, lusolve, transpose, exp, sqrt, erf } from 'mathjs';
...
var SurrogateModel = function () {
  function SurrogateModel() {
    classCallCheck(this, SurrogateModel);
  ...
  SurrogateModel.prototype.kernel = function kernel(X1, X2) {
    var _this = this;

    // Ensure inputs are 2D arrays
    var x1Matrix = Array.isArray(X1[0]) ? X1 : X1.map(function (x) {
      return Array.isArray(x) ? x : [x];
    });
    var x2Matrix = Array.isArray(X2[0]) ? X2 : X2.map(function (x) {
      return Array.isArray(x) ? x : [x];
    });

    return x1Matrix.map(function (x1) {
      return x2Matrix.map(function (x2) {
        // Ensure arrays have same dimensions before subtracting
        var x1Arr = Array.isArray(x1) ? x1 : [x1];
        var x2Arr = Array.isArray(x2) ? x2 : [x2];

        if (x1Arr.length !== x2Arr.length) {
          var maxLen = Math.max(x1Arr.length, x2Arr.length);
          while (x1Arr.length < maxLen) {
            x1Arr.push(0);
          }while (x2Arr.length < maxLen) {
            x2Arr.push(0);
          }
        }

        var diff = subtract(matrix(x1Arr), matrix(x2Arr));
        var squaredDist = sum(square(diff));
        return Math.exp(-0.5 * squaredDist / Math.pow(_this.lengthScale, 2));
      });
    });
  };

  SurrogateModel.prototype.fit = function fit(X, y) {
    if (!X.length || !y.length) return;

    this.X = X;
    this.y = Array.isArray(y) ? y : [y];

    var K = this.kernel(X, X);
    var kY = add(K, multiply(this.noise, identity(K.length)));

    try {
      // Use the standalone cholesky package
      this.L = cho(kY);
      // Convert L to the format expected by subsequent operations
      this.L = this.L.map(function (row) {
        var fullRow = new Array(kY.length).fill(0);
        row.forEach(function (val, j) {
          fullRow[j] = val;
        });
        return fullRow;
      });
      var yMatrix = matrix(this.y);
      this.alpha = lusolve(matrix(this.L), transpose(yMatrix));
    } catch (e) {
      // Fallback to identity matrix if decomposition fails
      this.L = identity(K.length);
      this.alpha = matrix(this.y);
    }
  };

  SurrogateModel.prototype.predict = function predict(XNew) {
    var _this2 = this;

    var kS = this.kernel(XNew, this.X);
    var kSS = this.kernel(XNew, XNew);
    var mu = kS.map(function (kSRow) {
      return multiply(kSRow, _this2.alpha);
    });
    var v = lusolve(this.L, transpose(kS));
    var cov = subtract(kSS, multiply(transpose(v), v));
    var stdv = cov.map(function (row, i) {
      return Math.sqrt(Math.max(row[i], 1e-9));
    });
    return { mean: mu.map(function (m) {
        return m[0];
      }), stdv: stdv };
  };
...
function expectedImprovement(mu, sigma, yBest) {
  ...
  return mu.map(function (m, i) {
    var s = sigma[i] + epsilon;
    var z = (yBest - m) / s;
    var phi = exp(-0.5 * (z * z)) / sqrt(2 * Math.PI);
    var Phi = 0.5 * (1.0 + erf(z / sqrt(2)));
    return s * (z * Phi + phi);
  });
}

var BayesianSearch = function (_BaseSpace) {
  inherits(BayesianSearch, _BaseSpace);

  function BayesianSearch() {
    classCallCheck(this, BayesianSearch);

    var _this3 = possibleConstructorReturn(this, _BaseSpace.call(this));

    _this3.choice = function (params, rng) {
      var options = params.options;

      var idx = rng.randrange(0, options.length, 1);
      var option = options[idx];
      return _this3.eval(option, { rng: rng });
    };

    _this3.randint = function (params, rng) {
      return rng.randrange(0, params.upper, 1);
    };

    _this3.loguniform = function (params, rng) {
      var low = params.low,
          high = params.high;
    ...
    _this3.qloguniform = function (params, rng) {
      var low = params.low,
          high = params.high,
          q = params.q;
    ...
    _this3.normal = function (params, rng) {
      var mu = params.mu,
          sigma = params.sigma;
    ...
    _this3.qnormal = function (params, rng) {
      var mu = params.mu,
          sigma = params.sigma,
          q = params.q;
    ...
    _this3.lognormal = function (params, rng) {
      var mu = params.mu,
          sigma = params.sigma;
    ...
    _this3.qlognormal = function (params, rng) {
      var mu = params.mu,
          sigma = params.sigma,
          q = params.q;
    ...
    _this3.uniform = function (params, rng) {
      var low = params.low,
          high = params.high;
    ...
    _this3.quniform = function (params, rng) {
      var low = params.low,
          high = params.high,
          q = params.q;
    ...
    _this3.surrogateModel = new SurrogateModel();
    _this3.observations = [];
    _this3.categoricalMaps = new Map(); // Store mappings for categorical variables
    return _this3;
  }
  ...
  BayesianSearch.prototype.updateSurrogateModel = function updateSurrogateModel() {
    var _this4 = this;

    var X = this.observations.map(function (obs) {
      return _this4.flattenParams(obs.params);
    });
    var y = this.observations.map(function (obs) {
      return obs.result.loss || obs.result.accuracy;
    });
    if (X.length > 0) {
  ...
  BayesianSearch.prototype.generateCandidates = function generateCandidates(expr, rng) {
    var numCandidates = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : 10;

    var candidates = [];
    var flatCandidates = [];
    for (var i = 0; i < numCandidates; i += 1) {
      var candidate = _BaseSpace.prototype.eval.call(this, expr, { rng: rng });
      candidates.push(candidate);
      flatCandidates.push(this.flattenParams(candidate));
    }
    return { candidates: candidates, flatCandidates: flatCandidates };
  };

  // Helper method to encode categorical values


  BayesianSearch.prototype.encodeCategorical = function encodeCategorical(value) {
  ...
  BayesianSearch.prototype.flattenParams = function flattenParams(params) {
    var _this5 = this;

    if (Array.isArray(params)) {
      return params.flatMap(function (p) {
        return _this5.flattenParams(p);
      });
    }
    if ((typeof params === 'undefined' ? 'undefined' : _typeof(params)) === 'object' && params !== null) {
      return Object.values(params).flatMap(function (p) {
        return _this5.flattenParams(p);
      });
    }
  ...
  BayesianSearch.prototype.propose = function propose(expr, rng) {
    var _generateCandidates = this.generateCandidates(expr, rng, 100),
        candidates = _generateCandidates.candidates,
        flatCandidates = _generateCandidates.flatCandidates;

    var _surrogateModel$predi = this.surrogateModel.predict(flatCandidates),
        mean = _surrogateModel$predi.mean,
        stdv = _surrogateModel$predi.stdv;

    var yBest = Math.min.apply(Math, this.observations.map(function (obs) {
      return obs.result.loss || Infinity;
    }));
    var eiValues = expectedImprovement(mean, stdv, yBest);

    var maxIndex = eiValues.indexOf(Math.max.apply(Math, eiValues));
  ...
  BayesianSearch.prototype.eval = function _eval(expr, _ref) {
    var rng = _ref.rng;
    ...
    return _BaseSpace.prototype.eval.call(this, expr, { rng: rng });
  };
...
var bayesianSample = function bayesianSample(space) {
  var params = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

  var bs = new BayesianSearch();
  var args = bs.eval(space, params);
  if (Object.keys(args).length === 1) {
    var results = Object.keys(args).map(function (key) {
      return args[key];
    });
...
var bayesianSearch = function bayesianSearch(newIds, domain, trials, seed) {
  var rng = new RandomState(seed);
  var rval = [];
  var bs = new BayesianSearch();

  // Update observations from past trials
  bs.observations = trials.trials.map(function (trial) {
    return {
      params: trial.args,
      result: trial.result
    };
  ...
  newIds.forEach(function (newId) {
    var paramsEval = bs.eval(domain.expr, { rng: rng });
    var result = domain.newResult();
    rval = [].concat(rval, trials.newTrialDocs([newId], [result], [paramsEval]));
  });
  return rval;
...
var search = {
  ...
  bayesianSearch: bayesianSearch
};

var sample = {
  randomSample: randomSample,
  gridSample: gridSample,
  bayesianSample: bayesianSample
};

export { fmin, RandomState, choice, randint, uniform, quniform, loguniform, qloguniform, normal, qnormal, lognormal, qlognormal, search, sample, STATUS_NEW, STATUS_RUNNING, STATUS_SUSPENDED, STATUS_OK, STATUS_FAIL, STATUS_STRINGS, JOB_STATE_NEW, JOB_STATE_RUNNING, JOB_STATE_DONE, JOB_STATE_ERROR, JOB_STATES, TRIAL_KEYS, range, Trials, Domain };
