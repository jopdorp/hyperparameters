...
typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('mathjs'), require('cholesky')) :
typeof define === 'function' && define.amd ? define(['exports', 'mathjs', 'cholesky'], factory) :
(factory((global.hpjs = {}),global.math,global.cho));
}(this, (function (exports,math,cho) { 'use strict';

cho = cho && cho.hasOwnProperty('default') ? cho['default'] : cho;
...
var SurrogateModel = function () {
  ...
  SurrogateModel.prototype.kernel = function kernel(X1, X2) {
    ...
    return x1Matrix.map(function (x1) {
      return x2Matrix.map(function (x2) {
        ...
        var diff = math.subtract(math.matrix(x1Arr), math.matrix(x2Arr));
        var squaredDist = math.sum(math.square(diff));
  ...
  SurrogateModel.prototype.fit = function fit(X, y) {
    ...
    var kY = math.add(K, math.multiply(this.noise, math.identity(K.length)));
    ...
    try {
      ...
      var yMatrix = math.matrix(this.y);
  ...
  SurrogateModel.prototype.predict = function predict(XNew) {
    ...
    var mu = kS.map(function (kSRow) {
      return math.multiply(kSRow, _this2.alpha);
    });
    var v = math.lusolve(this.L, math.transpose(kS));
    var cov = math.subtract(kSS, math.multiply(math.transpose(v), v));
...
function expectedImprovement(mu, sigma, yBest) {
  ...
  return mu.map(function (m, i) {
    ...
    var Phi = 0.5 * (1.0 + math.erf(z / math.sqrt(2)));
    return s * (z * Phi + phi);
  });
}
